...
namespace Solria.SAFT.Desktop.Models
{
	...
	public class SourceDocumentsPaymentsPaymentToolTipService
	{
		public SourceDocumentsPaymentsPaymentToolTipService()
		{
			...
			TransactionDate = string.Format("4.4.4.4 * Data{0}Data do recibo.{0}Data de emissão do recibo.", Environment.NewLine);
			PaymentType = string.Format("4.4.4.5 * Texto 2{0}Tipo de recibo.{0}Deve ser preenchido com:{0}«RC» - Recibo emitido no âmbito do regime de IVA de Caixa (incluindo os relativos a adiantamentos desse regime);{0}«RG» - Outros recibos emitidos.", Environment.NewLine);
			Description = string.Format("4.4.4.6 Texto 200{0}Descrição do pagamento.", Environment.NewLine);
			SystemID = string.Format("4.4.4.7 Texto 35{0}Numero gerado pela aplicação.{0}Número único do recibo gerado internamente pela aplicação.", Environment.NewLine);
			PaymentStatus = string.Format("4.4.4.8.1 * Texto 1{0}Estado atual do recibo.{0}Deve ser preenchido com:{0}«N» - Recibo normal e vigente;{0}«A» - Recibo anulado.", Environment.NewLine);
			PaymentStatusDate = string.Format("4.4.4.8.2 * Data e Hora{0}Data e hora do estado atual do recibo.{0}Data da última gravação do estado do recibo ao segundo.{0}Tipo data e hora: “AAAA-MM-DDThh:mm:ss”.", Environment.NewLine);
			Reason = string.Format("4.4.4.8.3 Texto 50{0}Motivo da alteração de estado do recibo.{0}Deve ser indicada a razão que levou à alteração de estado do recibo.", Environment.NewLine);
			ResponsableUserSourceID = string.Format("4.4.4.8.4 * Texto 30{0}Código do utilizador. Utilizador responsável pelo estado atual do recibo.", Environment.NewLine);
			SourcePayment = string.Format("4.4.4.9 * Texto 1{0}Origem  do  documento. Deve ser preenchido com:{0}«P» - Recibo produzido na aplicação;{0}«I» - Recibo integrado e produzido noutra aplicação;{0}«M» - Recibo proveniente de recuperação ou de emissão manual.", Environment.NewLine);
			PaymentMethod = string.Format("4.4.4.10{0}Forma de Pagamento.{0}Deve ser indicado o meio de pagamento utilizado. No caso de pagamentos mistos devem ser indicados os montantes  por tipo de meio e data de pagamento.", Environment.NewLine);
			PaymentMechanism = string.Format("4.4.4.10.1 * Texto 2{0}Meios de pagamento. Deve ser preenchido com:{0}«CC» - Cartão crédito;{0}«CD» - Cartão débito;{0}«CH» - Cheque bancário;{0}«CO» - Cheque ou cartão oferta;{0}«CS» - Compensação de saldos em conta corrente;{0}«DE» - Dinheiro eletrónico, por exemplo residente em cartões de fidelidade ou de pontos;{0}«LC» - Letra comercial;{0}«MB» - Referências de pagamento para Multibanco;{0}«NU» - Numerário;{0}«OU» - Outros meios aqui não assinalados;{0}«PR» - Permuta de bens;{0}«TB» - Transferência bancária ou débito direto autorizado;{0}«TR» - Ticket restaurante.", Environment.NewLine);
			PaymentAmount = string.Format("4.4.4.10.2 * Monetário{0}Montante do pagamento.{0}Deve ser indicado o montante por meio de pagamento.", Environment.NewLine);
			PaymentDate = string.Format("4.4.4.10.3 * Data{0}Data do pagamento.", Environment.NewLine);
			GeneratedDocumentUserSourceID = string.Format("4.4.4.11 * Texto 30{0}Código do utilizador.{0}Utilizador que gerou o documento.", Environment.NewLine);
			SystemEntryDate = string.Format("4.4.4.12 * Data e Hora{0}Data de gravação do recibo.{0}Data da gravação do registo ao segundo, Tipo data e hora: «AAAA-MM-DDThh:mm:ss».", Environment.NewLine);
			CustomerID = string.Format("4.4.4.13 * Texto 30{0}Identificador do cliente.{0}Chave única da tabela 2.2. — Tabela de clientes (Customer) respeitando a regra aí definida para o campo 2.2.1. — Identificador único do cliente (CustomerID).", Environment.NewLine);
		}
	...
	public class SourceDocumentsPaymentsPaymentLineToolTipService
	{
		...
		{
			LineNumber = string.Format("4.4.4.14.1 * Inteiro{0}Número de linha.{0}As linhas devem ser exportadas pela mesma ordem em que se encontram no recibo original.", Environment.NewLine);
			SourceDocumentID = string.Format("4.4.4.14.2 *{0}Referência ao documento de origem.{0}Existindo a necessidade de efetuar mais do que uma referência, este campo poderá ser gerado tantas vezes quantas as necessárias.", Environment.NewLine);
			OriginatingON = string.Format("4.4.4.14.2.1 * Texto 60{0}Número do documento de origem.{0}Deve ser indicado o tipo, a série e o número da fatura ou documento retificativo desta a que respeita o pagamento.{0}Se o documento referido estiver contido no SAF-T(PT) deve ser utilizada a estrutura de numeração do campo 4.1.4.1 - Identificação única do documento de venda (InvoiceNo) da Tabela 4.1. - Documentos comerciais a clientes (SalesInvoices).", Environment.NewLine);
			InvoiceDate = string.Format("4.4.4.14.2.2 * Data{0}Data do documento de origem.{0}Deve ser indicada a data da fatura ou documento retificativo desta a que se refere o pagamento.", Environment.NewLine);
			Description = string.Format("4.4.4.14.2.3 Texto 100{0}Descrição da linha.{0}Descrição da linha de recebimento.", Environment.NewLine);
			LineSettlementAmount = string.Format("4.4.4.14.3 Monetário{0}Montante do desconto da linha.{0}Descontos concedidos aquando do pagamento deste documento.", Environment.NewLine);
			DebitAmount = string.Format("4.4.4.14.4 ** Monetário{0}Valor a débito.{0}Valor desta linha do recibo do documento retificativo, sem impostos e eventuais descontos. ", Environment.NewLine);
			CreditAmount = string.Format("4.4.4.14.5 ** Monetário{0}Valor a crédito.{0}Valor desta linha do recibo da fatura ou documento retificativo, sem impostos e eventuais descontos.", Environment.NewLine);
			Tax = string.Format("4.4.4.14.6 {0}Taxa de imposto.{0}Nos recibos do sistema de IVA de Caixa, deve ser indicada uma linha por cada taxa de IVA diferente, que conste da fatura respetiva.", Environment.NewLine);
			TaxType = string.Format("4.4.4.14.6.1 * Texto 3{0}Código do tipo de imposto.{0}Neste campo deve ser indicado o tipo de imposto.{0}Deve ser preenchido com:{0}«IVA» - Imposto sobre o valor acrescentado, para o regime de IVA de Caixa;{0}«IS» - Imposto de Selo;{0}«NS» - Não sujeito a IVA ou IS.", Environment.NewLine);
			TaxCountryRegion = string.Format("4.4.4.14.6.2 * Texto 5{0}País ou região do imposto.{0}Deve  ser  preenchido  de  acordo  com  a  norma  ISO 3166-1-alpha-2.{0}No caso das regiões autónomas da Madeira e Açores deve ser preenchido com:{0}«PT-AC» - Espaço fiscal da Região Autónoma dos Açores;{0}«PT-MA» - Espaço fiscal da Região Autónoma da Madeira.", Environment.NewLine);
			TaxCode = string.Format("4.4.4.14.6.3 * Texto 10{0}Código da taxa.{0}Código da taxa na tabela de impostos.{0}No caso do campo 2.5.1.1 - Código do tipo de imposto (TaxType) = IVA, deve ser preenchido com:{0}«RED» - Taxa reduzida;{0}«INT» - Taxa intermédia;{0}«NOR» - Taxa normal;{0}«ISE» - Isenta;{0}«OUT» - Outros, aplicável para os regimes especiais de IVA.{0}No caso do campo 2.5.1.1 - Código do tipo de imposto (TaxType) = «IS», deve ser preenchido com o código da verba respetiva.{0}No caso de não aplicabilidade de imposto deve ser preenchido com «NA».", Environment.NewLine);
			TaxPercentage = string.Format("4.4.4.14.6.4 ** Decimal{0}Percentagem da taxa de imposto.{0}O preenchimento é obrigatório, no caso de se tratar de uma percentagem de imposto.{0}A percentagem da taxa é correspondente ao imposto aplicável ao campo 4.4.4.14.4 - Valor a débito (DebitAmount) ou ao campo 4.4.4.14.5 - Valor a crédito (CreditAmount).{0}No caso de isenção ou não sujeição a imposto, utilizar este campo com o valor «0» (zero).", Environment.NewLine);
			TaxAmount = string.Format("4.4.4.14.6.5 ** Monetário{0}Montante do imposto.{0}O preenchimento é obrigatório, no caso de se tratar de uma verba fixa de imposto de selo. ", Environment.NewLine);
			TaxExemptionReason = string.Format("4.4.4.14.6.6 ** Texto 60{0}Motivo da isenção de imposto.{0}O preenchimento é obrigatório, quando os campos 4.4.4.16.4 - Percentagem da taxa de imposto (TaxPercentage) ou 4.4.4.16.5 - Montante do imposto (TaxAmount) são iguais a zero.{0}Deve ser referido o preceito legal aplicável.{0}Este campo deve ser igualmente preenchido nos casos de não sujeição aos impostos referidos na tabela 2.5 - Tabela de impostos (TaxTable).", Environment.NewLine);
			DocumentTotals = string.Format("4.4.4.15 *{0}Totais do documento", Environment.NewLine);
			TaxPayable = string.Format("4.4.4.15.1 * Monetário{0}Valor do imposto a pagar.", Environment.NewLine);
			NetTotal = string.Format("4.4.4.15.2 * Monetário{0}Total do documento sem impostos.{0}Este campo não deve incluir as parcelas referentes aos impostos constantes da tabela 2.5 - Tabela de impostos (TaxTable).", Environment.NewLine);
			GrossTotal = string.Format("4.4.4.15.3 * Monetário{0}Total do documento com impostos.{0}Este campo não deve refletir eventuais retenções na fonte constantes no campo 4.4.4.18 — Retenção na fonte (WithholdingTax).", Environment.NewLine);
			Settlement = string.Format("4.4.4.16{0}Acordos sobre descontos nas formas de pagamento.", Environment.NewLine);
			TotalSettlementAmount = string.Format("4.4.4.16.1 * Monetário{0}Montante dos descontos.{0}Total dos descontos concedidos aquando deste pagamento.", Environment.NewLine);
			Currency = string.Format("4.4.4.17{0}Moeda.{0}Esta estrutura é obrigatória no caso da emissão do documento ser em moeda estrangeira.", Environment.NewLine);
			CurrencyCode = string.Format("4.4.4.17.1 * Texto 3{0}Código de moeda.{0}No caso de moeda estrangeira deve ser preenchido de acordo com a norma ISO 4217.", Environment.NewLine);
			CurrencyAmount = string.Format("4.4.4.17.2 * Monetário{0}Valor total em moeda estrangeira.{0}Valor do campo 4.4.4.15.3 - Total do documento com impostos (GrossTotal) na moeda original do documento.", Environment.NewLine);
			ExchangeRate = string.Format("4.4.4.17.3 * Decimal{0}Taxa de câmbio.{0}Deve ser indicada a taxa de câmbio utilizada na conversão para EUR.", Environment.NewLine);
			WithholdingTax = string.Format("4.4.4.18{0}Retenção na fonte.", Environment.NewLine);
			WithholdingTaxType = string.Format("4.4.4.18.1 Texto 3{0}Código do tipo de imposto retido.{0}Neste campo deve ser indicado o tipo de imposto retido, preenchendo-o com:{0}«IRS» - Imposto sobre o rendimento de pessoas singulares;{0}«IRC» - Imposto sobre o rendimento de pessoas coletivas;{0}«IS» - Imposto do selo.", Environment.NewLine);
		}
